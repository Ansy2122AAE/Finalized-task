...
function out = forces_moments(x, delta, wind, P)
    ...
    Va =sqrt(V_a_b(1)*V_a_b(1)+V_a_b(2)*V_a_b(2)+V_a_b(3)*V_a_b(3));
    ...
    aForce = [(0.5*P.rho*Va*Va*P.S_wing)*(C_X+C_X_q*(P.c/2*Va)*q+C_X_delta_e*delta_e);
    ...
    pForce = [(0.5*P.rho*P.S_prop*P.C_prop)*((P.k_motor*delta_t)*(P.k_motor*delta_t)-(Va)*(Va));0;0;];
    
    % Overall force 
    Force = gForce + aForce + pForce;

    % Torques due to aerodynamics and propulsion (ensure Torque is a column vector)
    Torque = [(0.5*P.rho*Va*Va*P.S_wing)*(P.b*(P.C_ell_0+P.C_ell_beta*beta+P.C_ell_p*(P.b/2*Va)*p+P.C_ell_r*(P.b/2*Va)*r+P.C_ell_delta_a*delta_a+P.C_ell_delta_r*delta_r));
